...
function startDraw (evt) {
    if (system.currentTool) {
        if (system.currentTool === 'brush') {
            drawLine (evt)
        } else if (system.currentTool === "brush2") {
            drawLongLine(evt)
        }
    } else {
        alert ('choose tool')
...
function drawLongLine (evt) {
    canvas.onmousemove = function (evt) {
        ctx.fillStyle = system.currentColor
        ctx.fillRect (+xCoordBlock.innerText, +yCoordBlock.innerText, +xCoordBlock.innerText, system.brushSize)
    }
}
