...
shoppingBag.forEach(el => {
    ...
    out += `<img src="${el.image}" class="card-img-top" alt="...">`
    out += `<div class="card-body">`
    out += `<h5 class="card-title fontsize"> ${el.name} </h5>`
    out += `<p class="card-text"> ${el.price} USD</p>`
    out += `<button class="btn btn-primary">Купить</button>`
    out += '</div>'
    out += '</div>'
})
